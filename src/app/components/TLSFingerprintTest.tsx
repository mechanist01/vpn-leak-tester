     ...
     {results && (
       <div className="ml-9 space-y-4 text-sm text-gray-600 font-mono">
         <div className="space-y-2">
           ...
           <div>Version: {results.tls_info.version}</div>
           <div>Cipher Suite: {results.tls_info.cipher_suite.name}</div>
           <div>Protocol: {results.tls_info.cipher_suite.protocol}</div>
           <div>Bits: {results.tls_info.cipher_suite.bits}</div>
         </div>

         <div className="space-y-2 pt-2 border-t border-gray-200">
           <h4 className="font-semibold">Certificate Information:</h4>
           <div>Subject: {results.tls_info.cert_info.subject}</div>
           <div>Issuer: {results.tls_info.cert_info.issuer}</div>
           <div>Valid From: {formatDate(results.tls_info.cert_info.not_valid_before)}</div>
           <div>Valid To: {formatDate(results.tls_info.cert_info.not_valid_after)}</div>
           <div>Serial Number: {results.tls_info.cert_info.serial_number}</div>
         </div>
       </div>
     )}
